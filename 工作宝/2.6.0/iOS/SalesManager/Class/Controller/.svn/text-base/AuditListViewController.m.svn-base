...
- (void)viewDidLoad {
    ...
    //数据源
    self.pullTableView = [[PullTableView alloc] initWithFrame:CGRectMake(0, 90, MAINWIDTH, MAINHEIGHT - 90)];
    ...
    tmpFrame.size.height = MAINHEIGHT - 75;
...
-(void)setFirstStatus {
    ...
    if (applyParams != nil){
        ...
        [pb setPage:1];
        [pb setAuditStatus:[NSString stringWithFormat:@"%d",APPLY_NOT_MANAGE]];
...
-(void)HeaderSearchBarClickBtn:(int)index current:(int)current andWithTag:(int)tag{
    ...
    if (tag == 9999) {
        ...
        ApplyItemParams_Builder* pb = [applyParams toBuilder];
        if (index == 0) {
           statusString = [NSString stringWithFormat:@"%d",APPLY_NOT_MANAGE];
           
        }else {
             statusString = [NSString stringWithFormat:@"%d",APPLY_MANAGE];
...
- (UITableViewCell *) tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    [cell.userImage setImageWithURL:[NSURL URLWithString:[w.filePath objectAtIndex:0]] refreshCache:NO placeholderImage:[UIImage imageNamed:@"ic_unknow_avatar"]];
