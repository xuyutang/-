...
static NSString* CREATE_TABLE_ANNOUNCE = @"create table if not exists announce (id integer primary key,subject text,readed char(1),status char(1), create_date datetime,deleted integer default 0);";
...
static NSString* ADD_USER_TABLE_MESSAGE = @"alert table message ADD userId integer,ADD realName varchar(50),ADD avtar varchar(500), ADD userName varchar(100), ADD type integer, ADD sourceId varchar(100)";
static NSString* ADD_DEPTID_TABLE_ALLUSER = @"alert table all_user add departmentId integer";
static NSString* ADD_AVTAR_TABLE_COMPANY = @"alert table company ADD idea varchar(200),ADD avtar varchar(200)";
...
- (BOOL) saveAnnounce:(Announce*) a{
    ...
    if ([db open])
    {
        ...
        NSString* sql = @"";
        sql = [NSString stringWithFormat:@"insert into announce (id,subject,create_date,readed,status) values (%d,'%@','%@','%@','%@');",a.id,a.subject,a.createDate,@"N",@"N"];
        [db executeUpdate:sql];
...
-(BOOL)updateAnnounceSender:(User*)sender;{
    ...
    if ([db open])
    {
        ...
        NSString * sql = [NSString stringWithFormat:@"update announce_sender set avtar = '%@' where id=%d;",avtar,sender.id];
