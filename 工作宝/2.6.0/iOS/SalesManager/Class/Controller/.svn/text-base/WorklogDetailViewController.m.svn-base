...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    if (indexPath.section == 0 && worklog.today.length > 0) {
        CGSize size = [super rebuildSizeWithString:worklog.today ContentWidth:MAINWIDTH FontSize:FONT_SIZE + 1];
        return  size.height < CELL_DEFAULT_HEIGHT ? CELL_DEFAULT_HEIGHT : size.height + 10;
    }
    if (indexPath.section == 1 && worklog.plan.length > 0) {
        CGSize size = [super rebuildSizeWithString:worklog.plan ContentWidth:MAINWIDTH FontSize:FONT_SIZE + 1];
        return  size.height < CELL_DEFAULT_HEIGHT ? CELL_DEFAULT_HEIGHT : size.height + 10;
    }
    if (indexPath.section == 2 && worklog.special.length > 0) {
        CGSize size = [super rebuildSizeWithString:worklog.special ContentWidth:MAINWIDTH FontSize:FONT_SIZE + 1];
        return  size.height < CELL_DEFAULT_HEIGHT ? CELL_DEFAULT_HEIGHT : size.height + 10;
    }
