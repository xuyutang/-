...
- (void)viewDidLoad
{
    ...
    UIImageView *saveImageView = [[UIImageView alloc] initWithFrame:CGRectMake(100, 18, 30, 25)];
...
-(void)choseRefreshParams {
    ...
    filterContacts = [[NSMutableArray alloc] init];
    for (int i = 0; i<[contacts count]; i++) {
        if ([_cusPhone isEqualToString:@"" ] && [_cusName isEqualToString:@""]) {
            [self _refresh];
            return;
        }

        User* user = [contacts objectAtIndex:i];
        NSRange rangeName = [user.realName rangeOfString:_cusName];
        NSRange rangeTel = [user.userName rangeOfString:_cusPhone];
